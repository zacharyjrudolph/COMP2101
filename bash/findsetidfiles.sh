...
do
  linex=$(($linex+1))
  find / -type f -executable -ls  2>/dev/null | sort -k 7 -nr | awk '{print $5}' | head -n 10 | sed -n $linex\p | tr -d '\n'
  echo "|MB:" | tr -d '\n'
  variablex=$(find / -type f -executable -ls  2>/dev/null | sort -k 7 -nr | awk '{print $7}' | head -n 10 | sed -n $linex\p)
  variabley=$((variablex / 1000000))
  echo "$variabley" | tr -d '\n'
  echo "|" | tr -d '\n'
  find / -type f -executable -ls  2>/dev/null | sort -k 7 -nr | awk '{print $11}' | head -n 10 | sed -n $linex\p
done
