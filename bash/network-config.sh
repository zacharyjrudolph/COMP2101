...
routername=$(cat /etc/hosts | sed -n 3p | awk '{print $2 }' )
routerip=$(cat /etc/hosts | sed -n 3p | awk '{print $1 }' )
networkip=$(cat /etc/networks | sed -n 3p | awk '{print $2 }' )
networkname=$(cat /etc/networks | sed -n 3p | awk '{print $1 }' )
echo "hotname : $hostname"
echo "LAN address :$lanaddress"
echo "LAN hostname : $lanhostname"
echo "external IP : $externalip"
echo "external name : $externalname"
echo "router info : $routername"
